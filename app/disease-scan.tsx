...
export default function DiseaseScanScreen() {
  ...
  const pick = async () => {
    ...
    if (!perm.granted) {
      toast.toast({ title: 'Photo access needed', variant: 'destructive' });
      return;
    }
  ...
  const analyze = () => {
    if (!uri || !farmId) {
      if (!farmId) toast.toast({ title: 'Select a farm first', variant: 'destructive' });
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    toast.toast({ title: 'Scan complete', variant: 'success' });
  };

  return (
    <OperationsScreen title="Disease scan" subtitle="Photo-based AI suspicion">
      <CountPillButton
        label="Upload photo"
        variant="outline"
        icon={Camera}
